  ...
  getCharacteristics(id){
    ...
    .then(data => {
      ...
      if(descriptionList.length > 0 || data === 'Not Found'){
          characteristic = { 
          description: data.descriptions.filter(element => element.language.name === 'en')[0].description
        };
     }else{
          characteristic = { 
  ...
  render(){
    return (
      <div className="App">
        ...
        <DetailView pokemon={ this.state.pokemon } pokemonCharacteristic={this.state.pokemonCharacteristic}/>
      </div>
    )
  }
}
